...
const UserPenItem = ({ pen, myid, ismypen }) => {
  return (
    <>
      <Link href={`/pen/${pen.id}`} className="relative penItem_bg rounded-xl flex flex-col p-4 gap-4 pb-2">
        <header className="flex justify-between items-center">
          <HeaderPen created_at={pen.created_at} pen_id={pen.id} ismypen={ismypen} />
        </header>
